...
def cnn_network(queryfeatures, passagefeatures, num_classes):
    with C.layers.default_options(activation=C.ops.relu, pad=False):
        convA1 = C.layers.Convolution2D((6,10),4,pad=False,activation=C.tanh,name='convA1')(queryfeatures) #input : 15*50 #output : 4*10*41
        ...
        convB1 = C.layers.Convolution2D((21,21),4,pad=False,activation=C.tanh,name='convB1')(passagefeatures) #input : 120*50  #output : 4*100*41
        poolB1 = C.layers.MaxPooling((5,5),(5,5),name='poolB1')(convB1) #output : 4*20*8
        convB2 = C.layers.Convolution2D((11,3),2,pad=False,activation=C.tanh,name='convB2')(poolB1) #output : 2*10*6
        poolB2 = C.layers.MaxPooling((2,2),(2,2),name='poolB2')(convB2) #output : 2*5*3
...
def TrainAndValidate(trainfile):
    ...
    total_epochs =  31#Total number of epochs to run
    ...
    model_output.restore('CNN120.dnn')
    ...
    learner = C.adam(model_output.parameters, lr=lr_per_minibatch, momentum = C.momentum_schedule(0.9, minibatch_size = minibatch_size))
    ...
    for epoch in range(total_epochs):       # loop over epochs
        ...
        model_output.save("CNN120_{}.dnn".format(epoch+19)) # Save the model for every epoch
        '''
...
if __name__ == "__main__":
    ...
    submissionFileName = "PRanswer_120.tsv"
